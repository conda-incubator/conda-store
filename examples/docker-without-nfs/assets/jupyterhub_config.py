...
class CondaStoreSpawner(SimpleLocalProcessSpawner):
    async def options_form(self, spawner):
        ...
        options = [
            f'<option value="{_["current_build_id"]}" {"selected" if i == 0 else ""}>{_["namespace"]["name"]}/{_["name"]}</option>'
...
"""
    ...
    async def start(self):
        ...
        self.args = [
            "--JupyterApp.kernel_spec_manager_class",
            "nb_conda_store_kernels.manager.CondaStoreKernelSpecManager",
        ]
