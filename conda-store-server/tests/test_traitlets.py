...
def test_conda_store_server_enable_ui(conda_store_server_session):
    conda_store_server_session.enable_ui = False
    client = TestClient(conda_store_server_session.init_fastapi_app())
    response = client.get("/")
    assert response.status_code == 404

    conda_store_server_session.enable_ui = True
    client = TestClient(conda_store_server_session.init_fastapi_app())
    response = client.get("/")
    assert response.status_code == 200


def test_conda_store_server_enable_api(conda_store_server_session):
    conda_store_server_session.enable_api = False
    client = TestClient(conda_store_server_session.init_fastapi_app())
    response = client.get("/api/v1/")
    assert response.status_code == 404

    conda_store_server_session.enable_api = True
    client = TestClient(conda_store_server_session.init_fastapi_app())
    response = client.get("/api/v1/")
    assert response.status_code == 200


def test_conda_store_server_enable_registry(conda_store_server_session):
    conda_store_server_session.enable_registry = False
    client = TestClient(conda_store_server_session.init_fastapi_app())
    response = client.get("/v2/")
    assert response.status_code == 404

    conda_store_server_session.enable_registry = True
    client = TestClient(conda_store_server_session.init_fastapi_app())
    response = client.get("/v2/")
    assert response.status_code == 401  # unauthorized


def test_conda_store_server_enable_metrics(conda_store_server_session):
    conda_store_server_session.enable_metrics = False
    client = TestClient(conda_store_server_session.init_fastapi_app())
    response = client.get("/metrics")
    assert response.status_code == 404

    conda_store_server_session.enable_metrics = True
    client = TestClient(conda_store_server_session.init_fastapi_app())
    response = client.get("/metrics")
    assert response.status_code == 200


def test_conda_store_server_url_prefix(conda_store_server_session):
    conda_store_server_session.url_prefix = "/a/test/prefix/"
    client = TestClient(conda_store_server_session.init_fastapi_app())
    ...
    conda_store_server_session.url_prefix = "/"
    client = TestClient(conda_store_server_session.init_fastapi_app())
    response = client.get("/", follow_redirects=False)
    assert response.status_code == 200


def test_conda_store_server_templates(conda_store_server_session):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        conda_store_server_session.templates = templates
        conda_store_server_session.template_vars = {"test_variable": "Test"}
        client = TestClient(conda_store_server_session.init_fastapi_app())
...
def test_conda_store_server_authentication_class(conda_store_server_session):
    from conda_store_server.server.auth import DummyAuthentication

    class MyAuthentication(DummyAuthentication):
        password = "helloworld"

    conda_store_server_session.authentication_class = MyAuthentication
    conda_store_server_session.initialize()
...
def test_conda_store_server_additional_routes(conda_store_server_session):
    def a_route(a: str, b: str):
        return {"data": f"Hello World {a} {b}"}

    conda_store_server_session.additional_routes = [
        (
            "/a/path/to/another/",
            "get",
            a_route,
