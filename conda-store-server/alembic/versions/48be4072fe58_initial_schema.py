...
def upgrade():
    ...
    op.create_table('conda_store_configuration',
    sa.Column('id', sa.Integer(), nullable=False),
    ...
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('channel_id', 'subdir', 'name', 'version', 'build', 'build_number', 'sha256', name='_conda_package_uc')
    )
    ...
    sa.Column('status', sa.Enum('QUEUED', 'BUILDING', 'COMPLETED', 'FAILED', name='buildstatus'), nullable=True),
    ...
    op.create_foreign_key(constraint_name='fk_environment_current_build_id_build_id', 
    ...
    op.create_table('solve_conda_package',
    sa.Column('solve_id', sa.Integer(), nullable=False),
    sa.Column('conda_package_id', sa.Integer(), nullable=False),
    sa.ForeignKeyConstraint(['conda_package_id'], ['conda_package.id'], ondelete='CASCADE'),
    ...
    sa.Column('artifact_type', sa.Enum('DIRECTORY', 'LOCKFILE', 'LOGS', 'YAML', 'CONDA_PACK', 'DOCKER_BLOB', 'DOCKER_MANIFEST', name='buildartifacttype'), nullable=False),
    sa.Column('key', sa.Unicode(length=255), nullable=True),
    sa.ForeignKeyConstraint(['build_id'], ['build.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('build_conda_package',
